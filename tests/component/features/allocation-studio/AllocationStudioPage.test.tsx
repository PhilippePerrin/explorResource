...
describe('AllocationStudioPage', () => {
  ...
  it('lets keyboard users activate a board drop target with a prepared token', async () => {
    await resourceTypesRepository.put({
      id: 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
      label: 'Developer',
      shortCode: 'DEV',
    ...
    await user.selectOptions(
      within(dragTokenSection as HTMLElement).getByLabelText(/^Month$/i),
      '1',
    );
    await user.clear(within(dragTokenSection as HTMLElement).getByLabelText(/^Days$/i));
    await user.type(within(dragTokenSection as HTMLElement).getByLabelText(/^Days$/i), '3');

    const dropTarget = await screen.findByLabelText(
      /E0100 Commercial Analytics, month 1\. Focus and press Enter or Space to apply the prepared drag token\./i,
    );
    dropTarget.focus();
    await user.keyboard('{Enter}');
    expect(
      (await screen.findAllByText(/Prepared allocation added to E0100 for month 1\./i)).length,
    ).toBeGreaterThan(0);
    ...
    await waitFor(async () => {
      const allocations = await allocationsRepository.getAll();
      expect(allocations).toHaveLength(1);
      expect(allocations[0]?.allocatedDays).toBe(3);
    });
  });
});
