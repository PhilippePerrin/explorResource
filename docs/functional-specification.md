...
1. **Dashboard** — net capacity, allocated load, available capacity, global utilization rate, overloaded resources count, overload days, unallocated demand days, under-served projects count, demand variation since last import, monthly mini-trends, priority alerts, action shortcuts. A page-wide **filter bar** (Resource type / Focus — Year/S1/S2/Q1-Q4 / Year, URL-persisted with save/apply "favorites", same pattern as Allocation Studio) scopes **every** widget on the page — the hero utilization ring, the three KPI tiles, the Utilization trend chart, Priority alerts, the under-served-projects count, and the demand-variation panel — not just one chart. The **Alert focus month** picker (which selects the single month Priority alerts and its utilization badge describe) is retained but its options are restricted to the months inside the currently selected Focus period, and it transparently falls back to the first month of the period whenever the previously selected month falls outside a newly selected Focus/Year. All day-amount values shown anywhere on the page (priority alerts, demand variation, KPI tiles) are rounded to one decimal place, never a raw unrounded float.
...
8. **Non-working Days** — annual grid (resources × months), fast entry, paste-from-Excel, totals, validation.
   - **Import from Excel** (`/non-working-days/import`) — reads the team's "Team Calendar" workbook (PTO fill color, AM/PM half-day text, no cell comments), decodes each day against the sheet's own legend, matches resource names case-/accent-insensitively, and replaces the covered resource/month totals on the grid. Site closures/bank holidays and "Travelling" entries are never counted; anything the file doesn't explain is flagged, not guessed. Preview of staged monthly totals, anomaly review, atomic commit, downloadable Markdown report.
